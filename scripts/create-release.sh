#!/usr/bin/env bash
# Creates the v1.1.0 GitHub Release and aligns the repo description + topics.
#
# Prereqs:
...
#   git tag -a v1.1.0 -m "v1.1.0 — Inventarium" && git push origin v1.1.0
...
NOTES="$(dirname "$0")/../docs/release-notes-v1.1.0.md"

[ -f "$NOTES" ] || { echo "missing release notes: $NOTES" >&2; exit 1; }

gh release create "$TAG" \
  --repo "$REPO" \
  --title "v1.1.0 — Inventarium: the shared knowledge layer" \
  --notes-file "$NOTES"
