...
This is the **moat** week. Cinematic UI (Week 1) wins on first impression. Agent library (Week 2) wins on first useful run. Project knowledge wins on **retention** — the more you use inventarium, the more it knows your project, the worse it is to switch away.
...
- Footer line: `*Generated by inventarium · inventarium.dev*` (soft attribution)

Format:
```markdown
# {Board name} — Project knowledge

*Exported {date}. Generated by inventarium.*
...
- Existing CLAUDE.md files from inventarium itself import meaningfully
...
- [ ] At least 5 real proposals reviewed (accept or reject) on inventarium's own board this week
