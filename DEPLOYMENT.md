...
Run `agent-trail doctor` — the built-in preflight — to verify all of the above before you rely on the install.
...
- `./agent-trail.db` — SQLite (WAL mode)
- `./.agent-trail/replays/` — recorded SSE streams per execution
- `./.worktrees/` — per-task git worktrees (when the executor spawns them)
- `~/agent-trail-runs/` — fallback implementation dir (when a board doesn't set one)

To move the data, either `mv agent-trail.db somewhere/else` and set `AGENT_TRAIL_DB_PATH`, or launch from a different directory.
...
```bash
# From ANY directory (published to npm as `agent-trail`)
npx agent-trail
```
...
3. It spawns the server with `AGENT_TRAIL_ROOT = process.cwd()`.
...
EXPOSE 3002
ENV AGENT_TRAIL_PORT=3002 \
    AGENT_TRAIL_ROOT=/workspace \
...
```yaml
- name: Start agent-trail server
  run: |
    nohup npx agent-trail --no-open > /tmp/at.log 2>&1 &
    for _ in {1..30}; do curl -sf http://localhost:3002/api/health && break || sleep 1; done

- name: Run task headless
  env:
    ANTHROPIC_API_KEY: ${{ secrets.ANTHROPIC_API_KEY }}
  run: agent-trail run --task ${{ inputs.task_id }} --ci --timeout 1500 | tee "$GITHUB_STEP_SUMMARY"
```

Exit codes from `agent-trail run --ci`:
...
```bash
# systemd unit: /etc/systemd/system/agent-trail.service
[Unit]
Description=agent-trail
After=network.target

[Service]
Type=simple
User=trail
WorkingDirectory=/srv/agent-trail
Environment=AGENT_TRAIL_PORT=3002
Environment=AGENT_TRAIL_ROOT=/srv/agent-trail
Environment=AGENT_TRAIL_SKIP_RUNNER=0
...
**Auth is your responsibility.** agent-trail v1 does not ship SSO / RBAC — put it behind Cloudflare Access, an OAuth proxy (oauth2-proxy), or a Tailscale ACL.

---

## What agent-trail is NOT designed for
...
- **Read-only filesystem containers** — the agent needs write access to the working directory, the DB, and `.agent-trail/`. If your platform forbids writes, agent-trail won't work.
...
|-----------------------------|----------------------|-------------------------------------------------------------------------|
| `AGENT_TRAIL_PORT`          | `3002`               | HTTP server port                                                        |
| `AGENT_TRAIL_ROOT`          | CWD                  | Where DB, worktrees, `.agent-trail/` live                                |
| `AGENT_TRAIL_DB_PATH`       | `<root>/agent-trail.db` | Override the SQLite location outright                                |
| `AGENT_TRAIL_URL`           | `http://localhost:3002` | Client-side base URL (for the CLI)                                    |
| `AGENT_TRAIL_RUNNER_URL`    | `http://localhost:3003` | Dev-server manager URL (Phase 2 runner package)                       |
| `AGENT_TRAIL_SKIP_RUNNER`   | unset                | `=1` to skip auto-spawning the dev-server runner                        |
| `ANTHROPIC_API_KEY`         | unset                | Optional — the `claude` CLI uses its own auth if logged in              |

Legacy env vars (`VIBE_BOARD_DB_PATH`, `VIBE_BOARD_URL`, `VIBE_BOARD_RUNNER_URL`) are honored with a deprecation warning for one release. Migrate before v0.3.

Mock env vars used ONLY in tests — set them and the app runs without a real claude CLI, but the "runs" produce no real work:

- `AGENT_TRAIL_PLANNER_MOCK` — JSON or `file:<path>`
- `AGENT_TRAIL_CLAUDE_MOCK` — JSON scenario for the adapter
- `AGENT_TRAIL_CASE_GEN_MOCK` — JSON for the test-case generator
...
|------|-------------------------------------|------------------------------------|
| 3002 | Main HTTP server (SSE + static SPA) | `AGENT_TRAIL_PORT`                 |
| 3003 | Dev-server runner (managed dev)     | `AGENT_TRAIL_RUNNER_URL` + package  |
...
Everything is either a file in your project directory or a row in `agent-trail.db`. Backup strategy:

```bash
# Full snapshot — safe to run while agent-trail is stopped
tar czf agent-trail-backup-$(date +%F).tar.gz \
    agent-trail.db \
    .agent-trail/ \
    .worktrees/
```

If agent-trail is running, use SQLite's `.backup` command instead of copying the file directly (WAL mode makes a plain `cp` unsafe):

```bash
sqlite3 agent-trail.db ".backup 'agent-trail.db.snapshot'"
```

Retention:

- **Replay recordings** — `.agent-trail/replays/*.jsonl` grow with usage. Prune old ones via the API:
...
bun install
bun run -F @agent-trail/web build
sudo systemctl restart agent-trail   # or however you started it
```
...
|-----------------------------------------------|------------------------------------------------------------------------|
| Server dies on boot, "SQLITE_CANTOPEN"        | `AGENT_TRAIL_DB_PATH` points to a directory the user can't write to    |
...
| `agent-trail doctor` says claude CLI missing  | Install from https://claude.ai/download, then `claude login`           |
| Every run says `Cannot reach server`          | Port 3002 in use — either kill the old process or export `AGENT_TRAIL_PORT` |
...
For anything else, `bun run start` and check the logs — the server writes to stdout, and `.agent-trail/replays/<execId>.jsonl` has the exact SSE trace of a specific run.

---

## Getting help

- Bug or unexpected behavior → [open an issue](https://github.com/ahmd-nish/agent-trail/issues/new/choose)
