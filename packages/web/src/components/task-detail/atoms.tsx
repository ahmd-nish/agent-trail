...
import {
  ...
  Bot, ClipboardList, BookOpen, Settings, Rocket, Flame, Check,
  MessageSquare, PartyPopper, Skull, Hand, FlaskConical, Siren,
  ArrowRight, ArrowLeft, ChevronUp, ChevronDown, type LucideIcon,
} from "lucide-react";
...
export function SectionLabel({ children }: { children: ReactNode }) {
  return <p style={{ fontSize: 10, color: "var(--fg-faded)", letterSpacing: "0.06em", textTransform: "uppercase" as const, fontWeight: 500, marginBottom: 6 }}>{children}</p>;
}
...
export function StatusBadge({ status }: { status: TaskStatus }) {
  return (
    <span
      className="claw-chip"
      style={{ color: STATUS_COLOR[status], borderColor: `color-mix(in srgb, ${STATUS_COLOR[status]} 30%, transparent)` }}
    >
      {status.replace("_", " ")}
    </span>
  );
...
export function StatusDot({ status }: { status: TaskStatus }) {
  return <span className="w-1.5 h-1.5 rounded-full shrink-0" style={{ background: STATUS_COLOR[status] }} />;
}

// ─── Metric chip (review mode) ────────────────────────────────────────────────

export function MetricChip({ label, value, color }: { label: string; value: string; color: string }) {
  return (
    <div className="flex flex-col gap-0.5 px-4 py-3" style={{ background: "var(--bg-panel)", border: "1px solid var(--line)", borderRadius: 2 }}>
      <span style={{ fontSize: 10, color: "var(--fg-faded)", letterSpacing: "0.06em", textTransform: "uppercase" as const }}>{label}</span>
      <span className={`text-lg font-bold ${color}`} style={{ fontFamily: "inherit" }}>{value}</span>
    </div>
  );
}
...
export function DescriptionBullets({ text }: { text: string }) {
  if (!text.trim()) return <p className="text-sm text-slate-500 italic">No description provided.</p>;
  const lines = text
    .split("\n")
...
const TOOL_STYLE: Record<string, { Icon: LucideIcon; color: string; bg: string }> = {
  ...
  TodoWrite:    { Icon: ClipboardList, color: "text-yellow-300", bg: "bg-yellow-950/40" },
  NotebookEdit: { Icon: BookOpen,      color: "text-violet-300", bg: "bg-violet-950/40" },
};
const DEFAULT_TOOL = { Icon: Settings, color: "text-slate-300", bg: "bg-slate-800/50" };
...
export function LogLine({ event, isLatest = false }: { event: UiEvent; isLatest?: boolean }) {
  if (event.type === "connected") {
    ...
    return (
      <p className="log-line-enter flex items-center gap-2 text-slate-500">
        ...
        <span>Connected — session <span className="font-mono text-slate-400">{short}</span></span>
      </p>
    );
  }
  if (event.type === "tool_call") {
    ...
    return (
      <div className={`log-line-enter flex items-center gap-2 px-2 py-1 rounded ${s.bg} ${isLatest ? "live-glow" : ""}`}>
        <s.Icon size={14} className={`shrink-0 ${s.color}`} />
        <span className={`font-semibold ${s.color}`}>{event.tool}</span>
        {isLatest && <span className="ml-auto text-[10px] text-amber-400/70 animate-pulse">running…</span>}
      </div>
    );
  }
  if (event.type === "tool_result") {
    return (
      <p className={`log-line-enter flex items-center gap-2 pl-3 ${event.isError ? "text-red-400" : "text-emerald-500/80"}`}>
        {event.isError
          ? <Flame size={13} className="shrink-0" />
          : <Check size={13} className="shrink-0" />}
        <span className="text-[11px]">{event.isError ? "errored" : "done"}</span>
      </p>
    );
  }
  if (event.type === "text") {
  ...
  if (event.type === "execution_complete") {
    ...
    return (
      <div className={`log-line-enter font-semibold mt-2 px-3 py-2 rounded-lg border flex items-center gap-2 ${
        ok ? "border-emerald-700/50 bg-emerald-950/40 text-emerald-300" : "border-red-700/50 bg-red-950/40 text-red-300"
      }`}>
        {ok ? <PartyPopper size={15} className="shrink-0" /> : <Skull size={15} className="shrink-0" />}
  ...
  if (event.type === "test_result") {
    return (
      <div className={`log-line-enter font-semibold px-3 py-1.5 rounded flex items-center gap-2 ${
        event.passed ? "bg-emerald-950/40 text-emerald-300" : "bg-red-950/40 text-red-300"
      }`}>
        {event.passed ? <FlaskConical size={14} className="shrink-0" /> : <Siren size={14} className="shrink-0" />}
...
export function ActivityLine({ event }: { event: UiEvent }) {
  if (event.type === "connected") return <p className="text-[11px] text-slate-500">Connected — {event.executionId.slice(0, 8)}</p>;
  ...
  if (event.type === "tool_result") return (
    <p className={`text-[11px] flex items-center gap-1 ${event.isError ? "text-red-400" : "text-slate-500"}`}>
      <ArrowLeft size={10} className="shrink-0" />
      {event.isError ? "error" : "ok"}
    </p>
  );
  if (event.type === "text") return <p className="text-[11px] text-slate-300 leading-relaxed">{event.text}</p>;
  if (event.type === "execution_complete") return (
    <p className={`text-[11px] font-semibold flex items-center gap-1 ${event.status === "completed" ? "text-emerald-400" : "text-red-400"}`}>
...
export function ArtifactViewer({ artifact }: { artifact: ArtifactRow }) {
  ...
  const baseStyle: React.CSSProperties = { background: "var(--bg-panel)", border: "1px solid var(--line)", borderRadius: 2 };
  ...
  if (artifact.kind === "file_list") {
    return (
      <div className="px-3 py-2" style={baseStyle}>
        <p className="text-[10px] mb-1" style={{ color: "var(--fg-faded)" }}>{ARTIFACT_LABEL[artifact.kind]}</p>
        <pre className="text-[11px] whitespace-pre-wrap" style={{ color: "var(--fg-dim)", fontFamily: "inherit" }}>{artifact.content}</pre>
      </div>
    );
  }
  return (
    <div style={{ ...baseStyle, overflow: "hidden" }}>
      ...
      >
        <span className="text-[11px]" style={{ color: "var(--fg-dim)" }}>{ARTIFACT_LABEL[artifact.kind]}</span>
      ...
      {expanded && (
        <div className="max-h-64 overflow-y-auto" style={{ borderTop: "1px solid var(--line)" }}>
          {artifact.kind === "git_diff" ? <DiffViewer content={artifact.content} /> : (
            <pre className="text-[11px] p-3 whitespace-pre-wrap" style={{ color: "var(--fg-dim)", fontFamily: "inherit" }}>{artifact.content}</pre>
          )}
        </div>
      )}
    </div>
