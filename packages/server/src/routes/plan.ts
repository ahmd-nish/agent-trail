...
import { planFromPrd, type BoardDefaults } from "../../../core/src/planner/index.ts";
...
planRouter.post("/plan", async (c) => {
  ...
  try {
    result = await planFromPrd(body.prdText.trim(), boardId, boardDefaults);
  } catch (err) {
    ...
    const insert = db.prepare(
      `INSERT INTO tasks
         (id, board_id, title, description, status, priority, assignee, tdd_enabled, tdd_phase,
          ...
          model, model_tier, component, external_dependencies, test_cases, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    );
